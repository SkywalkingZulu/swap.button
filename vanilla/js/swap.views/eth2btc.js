	...
	APP.SwapViews['ETH2BTC'] = function () {
		...
		if (flow.step === 1) {
			...
			if (flow.isSignFetching || flow.signTransactionHash) {
				root.addVar('root',"<h4>Please wait. Confirmation processing</h4>");
		...
		if (flow.isMeSigned) {
			root.addVar('root',"<h3>Waiting BTC Owner creates Secret Key, creates BTC Script and charges it</h3>");
			if (flow.step === 2) {
				root.addVar('root',"<div><b>Wait...</b></div>");
			};
			if (flow.secretHash) {
