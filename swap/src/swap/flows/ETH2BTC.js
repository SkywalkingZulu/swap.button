...
class ETH2BTC extends Flow {
  ...
  _getSteps() {
    ...
    return [
      ...
      async () => {
        const { participant, buyRequestAmount } = this.swap

        const swapData = {
          myAddress:            storage.me.eth.address,
          participantAddress:   participant.eth.address,
          secretHash:           flow.state.secretHash,
          amount:               buyRequestAmount,
        }
  ...
  async syncBalance() {
    const { buyRequestAmount } = this.swap
    ...
    const isEnoughMoney = buyRequestAmount <= balance
