      ...
      <div>
        {
          swap.id && (
            swap.isMy ? (
              <strong>{swap.buyRequestAmount} {swap.sellCurrency} &#10230; {swap.buyRequestAmount*swap.exchangeRate} {swap.buyCurrency}</strong>
            ) : (
              <strong>{swap.buyRequestAmount*swap.exchangeRate} {swap.buyCurrency} &#10230; {swap.buyRequestAmount} {swap.sellCurrency}</strong>
            )
          )
        }
            ...
            <Fragment>
              ...
              </div>
              {
                !flow.isSignFetching && !flow.isMeSigned && (
                  new Date() - flow.timeStep < 10000 ? (
                    <Fragment>
                      <br />
                      <button onClick={this.signSwap}>Confirm</button>
                    </Fragment>
                  ) : (
                    <h3 style={{color:'red'}}>Вы не подтвердили транзакцию в отведенное время</h3>
                  )
                )
              }
              {
